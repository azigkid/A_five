...
class AIThread(QThread):
    ...
    def run(self):
        ...
        state = [[0]*15 for _ in range(15)]
        for i in range(15):
            for j in range(15):
                state[i][j]=self.chessboard[i][j][2]
        ai= a_state(state)
        temp=ai.next_move(2)
        record[0]=temp[0]
        record[1]=temp[1]
...
if __name__ == "__main__":
    ...
    # 设置标题
    myWin.setWindowTitle('A*算法求解五子棋')
    # 将窗口控件显示在屏幕上
    myWin.show()
