...
class AI:
    ...
    def ai(self, color, deep, pre_evaluate):
        # evaluateBoard函数评价当前棋盘上仅考虑某一种颜色的得分
                ...
                while j < len(directions):
                    ...
                    while k < len(directions_2):
                        ...
                        for i in range(4):
                            ...
                            if i == 1 and len(record) == 2:
                                if record[0] != record[1] and record[0] and record[1]:
                                    values += 10
                            if x + directions_2[k][0] < 0 or x + directions_2[k][0] > self.size - 1 or y + \
                                    directions_2[k][
                        ...
                        if len(record) == 5:
                            ...
                            if (count == 1 and record[1] == 0 and record.count(color) == 4) or (
                                    count == 1 and record[3] == 0 and record.count(color) == 4):
                                values += 3000
